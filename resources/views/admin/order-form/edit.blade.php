...
@section('content')
    <div data-aos="fade-up" data-aos-duration="1000" class="p-sm-30 p-15">
        <form class="ajax" action="{{ route('admin.order-form.store') }}" method="POST" data-handler="getShowMessage">
            @csrf
            <input type="hidden" name="service_id" value="{{ encrypt($orderForm->id) }}">
            <div class="max-w-894 m-auto">
                    ...
                    <div class="max-w-713 m-auto py-sm-52 py-15">
                        <div class="row rg-20">
                            <div class="col-12">
                                ...
                                <input type="text" name="service_name" value="{{ $orderForm->name }}"
                                    class="form-control zForm-control" id="addOrderFormFormName"
                            ...
                            <div class="col-12">
                                ...
                                <textarea name="service_details" id="addOrderFormInformation" class="form-control zForm-control min-h-175"
                                    placeholder="{{ __('Form Information') }}">{{ $orderForm->details }}</textarea>
                            </div>
                            <div class="col-12">
                                <div class="d-flex justify-content-between align-items-center flex-wrap g-10 pb-8">
                                ...
                                <input type="text" name="policy_link" value="{{ $orderForm->policy_link }}"
                                    class="form-control zForm-control" id="addOrderFormPolicyLink"
                                    placeholder="{{ __('Policy Link') }}" />
                            </div>
                            <div class="col-12 table-responsive">
                                <table class="table zTable zTable-last-item-right" id="inputTable">
                                    ...
                                    <tbody id="serviceItems">
                                        @foreach ($orderFormServices as $orderFormService)
                                            <tr>
                                                <input type="hidden" name="form_service_ids[]"
                                                    value="{{ $orderFormService->id }}">
                                                <td>
                                                    <select class="form-select service_id"
                                                        name="service_ids[]">
                                                        <option value="">{{ __('Select Option') }}</option>
                                                        @foreach ($services as $key => $service)
                                                            <option value="{{ $service->id }}"
                                                                data-price="{{ $service->price }}"
                                                                {{ $service->id == $orderFormService->service_id ? 'selected' : '' }}>
                                                ...
                                                <td>
                                                    <div class="min-w-100">
                                                        <input type="number" name="quantity[]" step="any"
                                                            min="0" value="{{ $orderFormService->quantity }}"
                ...
                <div class="pt-25 d-flex align-items-center flex-wrap g-10 order-form-buttons">
                    <button
                        class="bd-one bd-c-main-color bd-ra-8 bg-main-color py-10 px-26 fs-15 fw-600 lh-25 text-white saveService">{{ __('Update') }}</button>
