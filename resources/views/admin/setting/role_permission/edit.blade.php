            ...
            <div class="col-xl-9">
                <form class="ajax reset" action="{{route('admin.setting.role-permission.store')}}" method="POST"
                    ...
                    <div class="p-sm-25 p-15 bd-one bd-c-stroke bd-ra-10 bg-white mb-25">
                        <input type="hidden" value="{{encrypt($roleData->id)}}" name="id">
                        <div class="row rg-20">
                            <div class="col-12">
                                <label for="addRoleName" class="zForm-label">{{__("Name")}}</label>
                                <input type="text" class="form-control zForm-control" id="addRoleName" placeholder="{{__("Enter Name")}}"  name="name" value="{{$roleData->name}}"/>
                            </div>
                        </div>
                    </div>
