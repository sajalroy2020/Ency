                ...
                <div class="email-inbox__area bg-style">
                    <form class="ajax" action="{{route('super-admin.setting.settings_env.update')}}" method="POST"
                        ...
                        <div class="form-group text-black row mb-3">
                            ...
                            <div class="col-lg-9">
                                <select name="google_login_status" id="google_login_status" class="form-control">
                                    ...
                                    <option value="1" @if(getOption('google_login_status')==STATUS_ACTIVE) selected
                                        @endif>{{ getStatus(STATUS_ACTIVE) }}</option>
                                    <option value="0" @if(getOption('google_login_status') !=STATUS_ACTIVE) selected
                                        @endif>{{ getStatus(STATUS_DISABLE) }}</option>
                                </select>
                            </div>
                        </div>
                        <div class="form-group text-black row mb-3">
                        ...
                        <div class="form-group text-black row mb-3">
                            ...
                            <div class="col-lg-9">
                                <select name="facebook_login_status" id="facebook_login_status" class="form-control">
                                    <option value="">--{{ __('Select option') }}--</option>
                                    <option value="1" @if(getOption('facebook_login_status')==STATUS_ACTIVE) selected
                                        @endif>{{ getStatus(STATUS_ACTIVE) }}</option>
                                    <option value="0" @if(getOption('facebook_login_status') !=STATUS_ACTIVE) selected
                                        @endif>{{ getStatus(STATUS_DISABLE) }}</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="justify-content-end row text-end">
                            <div class="col-md-12">
                                <button type="submit"
                                    class="fs-15 fw-500 lh-25 text-black py-10 px-26 bg-cdef84 bd-ra-12 hover-bg-one float-right">{{__('Update')}}</button>
                            </div>
                        </div>
                    </form>
                </div>
...
<script src="{{ asset('assets/js/social-login.js') }}"></script>
@endpush
