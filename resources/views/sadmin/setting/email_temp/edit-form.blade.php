...
<form class="ajax reset" action="{{ route('admin.setting.email-temp-update', $template->id) }}" method="post"
    ...
    <div class="modal-body model-lg">
        <div class="row">
            <div class="col-md-12 mb-25">
                <p class="alert-success p-20">{{__("Email Template Fields")}} : @foreach(emailTempFields() as
                    $key=>$item) {{$key}} @endforeach</p>
            </div>
    ...
    <div class="modal-footer">
        <button type="submit"
            class="fs-15 fw-500 lh-25 text-black py-10 px-26 bg-cdef84 bd-ra-12 hover-bg-one border-0">{{ __('Save')
            }}</button>
    </div>
</form>