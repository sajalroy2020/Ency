...
class DesignationService
{
    ...
    {
        $data = Designation::where('user_id', auth()->id())->orderByDesc('id');
            ...
            ->addColumn('action', function ($data) {
                return "<div class='dropdown dropdown-one'>
                            ...
                            <ul class='dropdown-menu dropdownItem-two'>
                                <li>
                                    <a class='d-flex align-items-center cg-8' href='" . route('admin.setting.designation.edit', encrypt($data->id)) . "'>
                                        <div class='d-flex'>
                                ...
                                <li>
                                    <a class='d-flex align-items-center cg-8 delete' type='button' data-url='" . route('admin.setting.designation.delete', encrypt($data->id)) . "'>
                                        <div class='d-flex'>
    ...
    {
        return Designation::where('user_id', auth()->id())->find(decrypt($id));
    }
    ...
    public function store($request)
    {
        ...
        try {
            $id = $request->get('id', '');
            if ($id) {
                $designation = Designation::where('user_id', auth()->id())->findOrFail(decrypt($id));
            } else {
    ...
    {
        try {
            $data = Designation::where('user_id', auth()->id())->findOrFail(decrypt($id));
            $data->delete();
